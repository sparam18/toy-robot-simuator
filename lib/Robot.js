...
Robot.prototype.place = function(paramList) {
  ...
  if (x > this.tableSize.x || y > this.tableSize.y) {
    console.log(clc.red('Robot cannot be placed outside of the table.'));
    return this;
  }
...
Robot.prototype.move = function() {
  ...
  if (!this.isPlaced) {
    console.log(clc.red('Robot is not placed on the table.'));
    return this;
  }
  ...
  switch (this.direction) {
    case 'north':
      if (++y < this.tableSize.y) {
        ...
        console.log(clc.yellow('Robot cannot moved out of the table :('));
      }
      break;
    case 'east':
   ...
   //console.log(clc.blue('Robot position - '+ this.position.x +','+this.position.y +','+this.direction));
...
Robot.prototype.runInstructions = function(instructionList) {
  ...
  for (var i = 0; i<instructionList.length; i++) {
    ...
    console.log(clc.green('('+i+') Robot executing - '+ JSON.stringify(instruction)));    
    if (instruction.args) {
